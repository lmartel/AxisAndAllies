...
calcSpeed = function(unit){
    if(isDisrupted(unit)) return 0;
    var speed = getCard(unit).speed;
    if(isDamaged(unit)) speed--;
    return speed;
};
...
// Determine if the unit has an active status: disrupted, damaged, or both.
// "Destroyed" status will never be active, since activating it removes the unit from play.
...
getHighlightArgsForStatus = function(status, over, bold){
    var color;
    var opacity = 0.1;
    switch(status){
...
calcFractionalRound = function(game){
    var base = game.round;
    switch(game.phase){
        case Phase.MOVEMENT:
            return base;
        case Phase.ASSAULT:
            base += 0.3;
            if(!game.isFirstPlayerTurn) base += 0.3;
            return base;
        default:
            throw "unhandled phase in calcFractionalRound";
    }
};