...
(function(){
    ...
    function loadClientCode(){
        ...
        function Hexagon_draw(){
            if(this.animationActive) return this;
        ...
        function Hexagon_movePayloadAlongPath(path, options){
            ...
            var startingHex = this;
            var fullPath = [startingHex].concat(path);
            var targetHex = path[path.length - 1];

            // Soft-disable .draw() to prevent interruption of animation
            for(var i = 0; i < fullPath.length; i++){
//                fullPath[i].draw = function(){
//                    throw "DRAWERROR"
//                    return this;
//                };
                  // fullPath[i].animationActive = true;
            }

            var asset = startingHex.detachDrawnAsset();
                    ...
                    .each("end", function(){
                        if(i === path.length - 1){
                            for(var j = 0; j < fullPath.length; j++){
                                //delete fullPath[j].draw;
                                // fullPath[j].animationActive = false;
                            }
                            console.log("reattaching");
                            targetHex.reattachAsset(asset).draw();
